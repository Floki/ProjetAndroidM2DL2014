...
public class Score {
	...
	/**
	 * Compute the score from a gameShape This manage score compute and
	 * displayed text
	 * 
	 * @param shape
	 *            the Game shape with which we will compute the score
	 */
	public void computeScoreFromShape(GameShape shape) {
		if (shape.stillUse()) {
			...
			if (shape.isGoodMoment() || shape.isBonus()) { // a bonus is always a bonus because we are nice developers :)
				...
				this.score -= shape.getScore() * Game.level.getTooLatePercent()
						/ 100;
				comboLate(shape);
			}
